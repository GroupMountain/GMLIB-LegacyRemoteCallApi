...
void ExportGMLib_ModAPI() {
    /*
       ...
       //伤害类型
       RemoteCall::exportAs("GMLib_ModAPI", "registerDamageCause", [](int vanilla_cause, std::string cause_name,
       std::string direct_killer_type, std::string die_type) -> void {
           GMLib_Mod::addDamageCause((ActorDamageCause)vanilla_cause, cause_name, direct_killer_type, die_type);
       });
       RemoteCall::exportAs("GMLib_ModAPI", "translateResourcePack", [](std::string key1, std::vector<std::string> key2)
       -> std::string { return GMLib_Mod::i18nTranslate(key1, key2);
       });
